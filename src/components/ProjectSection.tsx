        ...
        <section className="gap-4 grid grid-cols-6 mx-4 mt-6" ref={ref}>
            ...
            <div className="relative col-span-6">
                <a
                    href={webPage}
                    target="_blank"
                    aria-label="See project webpage"
                    className="absolute inset-0 bg-yellow opacity-60 m-auto rounded-xl md:max-w-3/4 lg:h-100"
                ></a>
                <img src={imgSrc} alt={alt} className="m-auto rounded-xl md:max-w-3/4 lg:h-100" />
                <div className="lg:top-8 lg:right-10 absolute inset-0 lg:inset-auto bg-grey active:opacity-0 m-auto lg:m-0 p-3 rounded-xl lg:w-100 max-w-5/6 md:max-w-2/3 h-fit transition duration-300 ease-in-out delay-150">
                    <a href={webPage} target="_blank" aria-label="See project webpage" className="md:hidden block">
                        <p className="px-4 py-2 lg:text-md text-xs sm:text-base">{description}</p>
                    </a>
                    <p className="hidden md:block px-4 py-2 lg:text-md text-xs sm:text-base">{description}</p>
                </div>
            </div>
            <div className="relative col-span-2 lg:col-span-1 col-start-2 lg:col-start-3">
            ...
            <div className="flex flex-wrap gap-2 col-span-3 pl-1">
                {buildWith.map((el, i) => (
                    <span key={i} className="bg-grey p-2 rounded-xl w-fit text-xs xs:text-sm">
                        {el}
                    </span>
                ))}
            </div>
